...
var c;
// Represents the calendar view. Initialized by calendar_onload()
...
function delete_event(jqobj, calendar, e) {
	...
	jqobj.fullCalendar('removeEvents', function(e1) {
		return e === e1;
	});
}
...
function event_dropped(calendar) {
	...
	return function(event, dayDelta, minuteDelta, allDay, revertFunc) {
		if ('schedule_block_id' in event && !moved_events.some(function(e) {
...
function generate_schedule_proposal(calendar) {
	return {
	new: calendar.new_events.map(event_to_schedule_block).map(hack_around_backend_bug),
	moved: calendar.moved_events.map(event_to_schedule_block).map(hack_around_backend_bug),
	deleted:
	calendar.deleted_block_ids
};
}
...
function calendar_go_to_block(sb) {
	$("#schedule-content").fullCalendar("gotoDate", VUB_time_to_date(sb.week, sb.day, sb["first-slot"]));
}
